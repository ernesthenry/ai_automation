#!/usr/bin/env python3
"""Smoke tests: RAG + loan always; DSPy/OpenAI paths only if ``OPENAI_API_KEY`` is set."""
...
def main() -> None:
    ...
    if not key:
        print("smoke_test: RAG + loan OK; skipping OpenAI-backed checks (set OPENAI_API_KEY).")
        return

    from dspy_ticket_classifier.classifier import classify

    c1 = classify("Production checkout is down since morning.")
    assert "category" in c1 and c1["category"] in {"billing", "bug", "feature", "account"}, c1
    ...
    br = bc.post(
        ...
        json={
            ...
            "success_metrics": "Five verified answers",
        },
    )
    assert br.status_code == 200 and "brief" in br.json(), br.text

    print("smoke_test: all checks passed (including OpenAI-backed labs)")
